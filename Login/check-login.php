...
if (isset($_POST['username']) && isset($_POST['password']) && isset($_POST['role'])) {
	...
	// Q5: Check if the password is null. If null, redirect to index.php and print the error message
		...
		// Q6: Retrieve data from table customer for user authentication
		...
		if (mysqli_num_rows($result) === 1) {
			...
			if ($row['password'] === $password && $row['role'] === $role) {
				...
				// Q10: If match is false, redirect to index.php with error message
				header("Location: ../index.php?error=Incorrect username or password");
				exit();
			}
		} else { 
			// Q11: If username does not exist, redirect to index.php with error message
			header("Location: ../index.php?error=Incorrect username or password");
			exit();
		}
	}
} else { 
	// Q12: If username and password and role are null, redirect to index.php to enforce login
